...
](#why-you-cant-update-props-in-react)                                                                                                                                                      |
...
](#is-it-possible-to-use-asyncawait-in-plain-react)                                                                                                                            |
...
](#how-to-pass-params-to-historypush-method-in-react-router-v4)                                                                                            |
...
](#what-is-use-reducer-hook-can-you-describe-its-usage)                                                                                                               |
| 266 | [How do you compare useState and useReducer?
](#how-do-you-compare-use-state-and-use-reducer)                                                                                                                                     |
...
| 283 | [How does useContext work? Explain with an example
...
](#whats-a-common-pitfall-when-using-usecontext-with-objects)                                                                                                        |
...
| 300 | [What are the common use cases of useRef hook?
...
| 308 | [How does React Fiber work? Explain in detail
...
](#what-is-the-difference-between-super-and-superprops-in-react-using-es6-classes)                     |
...
](https: //github.com/jordwalke), a software engineer at Facebook (now Meta). It was first deployed on Facebook's News Feed in 2011 and on Instagram in 2012. The library was open-sourced in May 2013 and has since become one of the most popular JavaScript libraries for building modern user interfaces.
...
7.  ### When to use a Class Component over a Function Component?
    ...
    **Note:** You can also use reusable [react error boundary
](https: //github.com/bvaughn/react-error-boundary) third-party component without writing any class. i.e, No need to use class components for Error boundaries.
...
9.  ### What is state in React?
    ...
    ] = useState("Welcome to React world");
...
25. ### What are Higher-Order Components?
    ...
    **withAuth HOC Example:**
    ...
    import React from 'react';
    import { Navigate
} from 'react-router-dom'; // For redirection (assuming React Router v6)
...
45. ### What is the impact of indexes as keys?
    ...
    ```jsx harmony
    {
      todos.map((todo, index) => <Todo {...todo
    } key={index
    } />);
}
    ```
...
69. ### How can we find the version of React at runtime in the browser?
    ...
    ReactDOM.render(
      <div>{`React version: ${REACT_VERSION
    }`
}</div>,
...
74. ### Is it possible to use async/await in plain React?
    ...
    []);
      ...
      return user ? <div>Hello,
    {user.name
    }</div> : <div>Loading...</div>;
}
    ```
    But If you're not using a bundler like **Webpack or Babel**, you will need _Babel_ and [transform-async-to-generator
](https: //babeljs.io/docs/en/babel-plugin-transform-async-to-generator) plugin. However, React Native ships with Babel and a set of transforms.
...
83. ### How do you programmatically navigate using React Router v4?
    ...
    1.  **Using the `withRouter()` higher-order function:**
        ...
        ```jsx harmony
        import { withRouter
} from "react-router-dom"; // this also works with 'react-router-native'
...
92. ### What are the two ways of formatting in React Intl?
    ...
    "account"
          ...
          defaultMessage={
    "The amount is less than minimum balance."
}
        />
        ```
...
94. ### How to access current locale with React Intl?
    ...
    const MyComponent = ({ intl
      ...
      <div>{`The current locale is ${intl.locale
    }`
}</div>
    );
     ...
     const stringDate = this.props.intl.formatDate(date,
{
     ...
     const MyComponent = ({ intl
       ...
       <div>{`The formatted date is ${stringDate
    }`
}</div>
     );
...
161. ### How to pass numbers to React component?
     ...
     const ParentComponent = () => {
       return (
         <>
           <ChildComponent name="Chetan" age={
        30
    } />
         </>
       );
     ...
     function UserProfile() {
      ...
      return (
        <div>
          ...
          <p>Country: {country
    }</p> { /* This will throw an error if the name condition isn't met */}
        </div>
      );
}
     ```
...
//Example2:
    ...
    // Correct: Using a custom hook here
      ...
      return (
        <div>
          <h1>{user ? user.name : 'Loading profile...'
    }</h1>
        </div>
      );
}
...
208. ### How do you print falsy values in JSX?
     ...
     The falsy values such as false,
null, undefined, and true are valid children but they don't render anything. If you still want to display them then you need to convert it to string. Let's take an example on how to convert to a string,
...
220. ### Why do we use array destructuring (square brackets notation) in `useState`?
     ...
     When we declare a state variable with `useState`, it returns a pair — an array with two items. The first item is the current value, and the second is a function that updates the value. Using [
    0
] and [
    1
     ...
     var user = userStateVariable[
    0
]; // Access first item
     var setUser = userStateVariable[
    1
]; // Access second item
     ```
...
229. ### Should I learn ES6 before learning ReactJS?
     ...
     3. **Arrow functions:** Makes compact syntax
        ...
        // es 6
        const users = usersList.map((user) => <li>{user.name
}</li>);
        ```
...
244. ### What are the differences between Functional and Class Components?
     ...
     **A simple example showing how we use the setState() and constructor():**
     ...
     function App() {
       const [message, setMessage
    ] = useState("This is a functional component");
       const updateMessage = () => {
...
258.  ### How do you update arrays inside state?
    ...
    1
]='one'), push, pop, shift, unshift, splice etc methods should be avoided on original array. Instead, you can create a copy of existing array with help of array operations such as filter, map, slice, spread syntax etc.
...
285. ###  What's a common pitfall when using useContext with objects?
     ...
     **2. Memoize Context Value**  
     ...
     ```jsx
       const contextValue = useMemo(() => ({ user, setUser, theme, setTheme
}),
[user, theme
]);
     ```
...
286. ### What would the context value be for no matching provider?
     ...
     function ThemedComponent() {
       const theme = useContext(ThemeContext);
       return <div>Current theme: {theme
    }</div>;
}
...
[count
]); // Triggers again every time count updates
        ```
...
306. ### What are Custom React Hooks, and How Can You Develop One?
      ...
      **Custom Hooks** in React are JavaScript functions that allow you to **extract and reuse component logic** using React’s built-in Hooks like `useState`, `useEffect`, etc.
      ...
      function useFetchData(url) {
        const [data, setData
    ] = useState(null); // Holds the response
    ...
    } = useFetchData('https: //api.example.com/authors');
...
309. ### What is the useId hook and when should you use it?
     ...
     **Note:** The IDs generated by `useId` contain colons (`:`) which may not work in CSS selectors. For multiple related IDs, you can use the same `id` as a prefix: `${id
}-firstName`, `${id
}-lastName`.
...
310. ### What is the useDeferredValue hook?
     ...
     ```jsx
     import { useState, useDeferredValue, useMemo
} from 'react';
...
312. ### What is the useSyncExternalStore hook?
     ...
     function StatusBar() {
       ...
       return <h1>{isOnline ? '✅ Online' : '❌ Disconnected'
    }</h1>;
}
     ```
...
315. ### What is the useDebugValue hook?
     ...
     ```jsx
     import { useState, useEffect, useDebugValue
} from 'react';
    ...
    []);
     ...
     function useUser(userId) {
       ...
       useDebugValue(user, (user) => user ? `User: ${user.name
    }` : 'Loading...');
...
316. ### What are the differences between useEffect and useEvent (experimental)?
    ...
    [onTick
    ]); // Adding onTick causes interval to reset frequently
}
     ```
...
9. ### Why are String Refs legacy?
   ...
   If you worked with React before, you might be familiar with an older API where the `ref` attribute is a string, like `ref={'textInput'
}`, and the DOM node is accessed as `this.refs.textInput`. We advise against it because _string refs have below issues_, and are considered legacy. String refs were **removed in React v16**.
...
11. ### What are the lifecycle methods of React?
    ...
    - **getDerivedStateFromProps:** Invoked right before calling `render()` and is invoked on _every_ render. This exists for rare use cases where you need a derived state. Worth reading [if you need derived state
...
22. ### What will happen if you use props in initial state?
    ...
    class MyComponent extends React.Component {
      ...
      render() {
        return <div>{this.state.inputValue
        }</div>;
    }
}
    ```
    ...
    class MyComponent extends React.Component {
      ...
      render() {
        return <div>{this.props.inputValue
        }</div>;
    }
}
    ```
...
51. ### How to use `connect()` from React Redux?
    ...
    2.  **Connect the above props to your container:** The object returned by the `mapStateToProps` function is connected to the container. You can import `connect()` from `react-redux`.
        ...
        class App extends React.Component {
          render() {
            return <div>{this.props.containerData
        }</div>;
    }
}
...
53. ### How to use TypeScript in `create-react-app` application?
        ...
        Starting from react-scripts@3.3.0+ releases onwards, you can now optionally start a new app from a template by appending `--template [template-name
]` to the creation command. If you don't select a template, it will create your project with base template. Remember that templates are always named in the format `cra-template-[template-name
]`, here you only need to fill the `[template-name
]` section.
...
72. ### How do you say that state updates are merged?
    ...
    As mentioned in the above code snippets, `this.setState({comments
})` updates only comments variable without modifying or replacing `posts` variable.
...
74. ### How to prevent component from rendering?
    ...
    function Greeting(props) {
      ...
      return <div className="greeting">welcome,
    {props.name
    }</div>;
}
    ```
...
83. ### Is it possible to use react without JSX?
    ...
    class Greeting extends React.Component {
      render() {
        return <div>Hello {this.props.message
            }</div>;
        }
    }
